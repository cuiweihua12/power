...
var cwh = {
			...
			$.ajax({
				...
				error:function(err){
					if (err&& err.status && err.status == 403) {
						...
						cwh.powerConfirm(function(){
							window.location.href= getRootPath() + "/page/login.jsp";
						});
					}
				}
			});
		},
		upLoad:function(url,type,data,success){
			$.ajax({
				url:root_path + url,
				type:type,
				cache:false,
		...
		//初始化表格
		iniTable:function(table_obj,data_url,data_field,columns,queryParams){
		...
		relConfirm:function(success,title){
			swal({
    			title:title,
    			text:"删除后将无法恢复，请谨慎操作！",
    			type:"warning",
		...
		powerConfirm:function(success){
			swal({
				title:'你没有此权限是否退出,以管理员身份登录',
				text:"点击退出则跳转到登录页!",
				type:"warning",
				showCancelButton:true,
				confirmButtonColor:"#DD6B55",
		...
		cwhConfirm:function(success,title,text,btn){
			swal({
				title:title,
				text:text,
				type:"warning",
		...
		uploadImg:function(){
			...
			var webUploaderParams = {
					fileVal:"imgUrl",
					auto: true,
					server:cwh.getProjectUrl() + '/baseController/upload',
					pick : '#filePicker',
					fileNumLimit : 0,
					threads : 1,
					accept : {
